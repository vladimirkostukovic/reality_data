#Synchronizes the latest bezrealitky_* snapshot with the master table by inserting new rows, reactivating returned listings, archiving missing ones, and emitting a final JSON summary for the orchestrator.
...
def make_db_url() -> tuple[str, str]:
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Нет драйвера Postgres. Установи:\n"
...
# === EMIT FOR DEBUG (not used by wrapper) ===
...
def date_from_table_name(table_name: str) -> pd.Timestamp:
    d = table_name[-8:]
    return pd.to_datetime(f"{d[4:]}-{d[2:4]}-{d[:2]}")
...
def sync_bz_latest() -> None:
    ...
    try:
        ...
        # previous active rows BEFORE changes
        with engine.begin() as conn:
            prev_res = conn.execute(text("""
                SELECT COUNT(*)::bigint AS active_before
                FROM public.bezrealitky_standart
                WHERE available = TRUE
            """)).mappings().first()
        prev_active_rows = int(prev_res["active_before"]) if prev_res else 0
        ...
        with engine.begin() as conn:
            df = pd.read_sql_query(
                f'SELECT {select_cols} FROM public."{latest_table}"',
                con=conn
            )
        t_load = round(time.perf_counter() - t_load0, 3)
        ...
        df["available"] = True
        df = df[cols_insert]
        emit("source_loaded", rows=len(df), duration_s=t_load)
        ...
        # total rows AFTER
        with engine.begin() as conn:
            total_res = conn.execute(text("""
                SELECT COUNT(*)::bigint AS total_rows
                FROM public.bezrealitky_standart
            """)).mappings().first()
        total_rows = int(total_res["total_rows"]) if total_res else 0

        # FINAL SUMMARY JSON (wrapper format)
        ...
        final_log = {
            ...
            "source_rows": len(df),
            "prev_active_rows": prev_active_rows,
