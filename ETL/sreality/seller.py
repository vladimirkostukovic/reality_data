...
def main():
    ...
    if not tables:
        log.warning("no sreality_* snapshots found")
        print(json.dumps({"stage": "sync_summary", "stats": {"error": "no_snapshots"}}, ensure_ascii=False))
        print("SYNC COMPLETE")
        return
    ...
    if not _table_has_cols(latest, required):
        log.warning("%s missing required columns %s", latest, required)
        print(json.dumps({"stage": "sync_summary", "stats": {"snapshot_table": latest, "error": "missing_columns"}}, ensure_ascii=False))
        print("SYNC COMPLETE")
        return
    ...
    with engine.begin() as conn:
        ...
        insert_sql = text(f"""
            ...
            WHERE t.id IS NULL
              AND (n.agent_name IS NOT NULL OR n.agent_email IS NOT NULL OR n.agent_phone IS NOT NULL)
        """)
        res = conn.execute(insert_sql)
        inserted = res.rowcount or 0
        total_rows = int(conn.execute(text("SELECT COUNT(*) FROM public.sreality_seller")).scalar_one())

    log.info("source_rows=%d | inserted_new=%d | seller_rows_total=%d", src_rows, inserted, total_rows)
