...
# ---------- SQL (без точек с запятой встраиваемых фрагментов) ----------
...
SQL_CREATE_IDX_CONCURRENT += "/*+ CONCURRENTLY */"  # видимый маркер; см. исполнение в autocommit
# Примечание: в реальном SQL будет 'CREATE INDEX CONCURRENTLY IF NOT EXISTS ...'
...
def ensure_index_concurrently():
    # CREATE INDEX CONCURRENTLY доступен только вне транзакции
...
def delete_batch(conn, limit: int) -> int:
    ...
    # для DELETE с RETURNING rowcount — количество реально удалённых строк
    return res.rowcount or 0

def vacuum_analyze():
    # VACUUM только в autocommit
    with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
...
def main():
    ...
    # индекс до старта, чтобы не ловить долгие планы
    ...
    if to_del == 0:
        ...
        return  # stdout умышленно пустой
