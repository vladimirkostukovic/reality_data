...
# ====== RULES ======
PRICE_RATIO    = 1.20   # допуск цены (hi/lo <= 1.2)
AREA_REL_TOL   = 0.05   # 5% относительный допуск по площади
AREA_ABS_TOL   = 5.0    # 5 м² абсолютный допуск
...
def _geo_level(a: pd.Series, b: pd.Series) -> str | None:
    ...
    # сначала okres: если оба заданы и не равны — несовпадение
    ...
    # улица (дом не учитываем в уровне, дом часто пропадает)
    if as_ is not None and bs is not None and as_ != bs: return None

    # deepest level, где обе стороны заданы и равны
    if as_ is not None and bs is not None and as_ == bs:
        return "lvl1"
    if ap is not None and bp is not None and ap == bp:
        return "lvl2"
...
def _area_check(row1: pd.Series, row2: pd.Series) -> tuple[bool, str, float|None, float|None]:
    cat1 = row1.get("category_value"); cat2 = row2.get("category_value")
    land_pref1 = cat1 in CAT_LAND_VALUES if pd.notna(cat1) else False
    land_pref2 = cat2 in CAT_LAND_VALUES if pd.notna(cat2) else False

    a1b = row1.get("area_build"); a1l = row1.get("area_land")
    a2b = row2.get("area_build"); a2l = row2.get("area_land")

    # обе стороны имеют оба поля → успех, если совпал build или land
    both_have_both = (pd.notna(a1b) and pd.notna(a1l) and pd.notna(a2b) and pd.notna(a2l))
    if both_have_both:
        if _areas_close(a1b, a2b):
            return True, "build", float(a1b), float(a2b)
        if _areas_close(a1l, a2l):
            return True, "land",  float(a1l), float(a2l)
        return False, "mixed", None, None

    def pick_area(is_land_pref, ab, al):
        primary = al if is_land_pref else ab
        secondary = ab if is_land_pref else al
        if pd.notna(primary):   return float(primary), ("land" if is_land_pref else "build")
        if pd.notna(secondary): return float(secondary), ("build" if is_land_pref else "land")
        return None, "none"

    v1, m1 = pick_area(land_pref1, a1b, a1l)
    v2, m2 = pick_area(land_pref2, a2b, a2l)

    if v1 is None or v2 is None:
        return False, "none", None, None

    ok = _areas_close(v1, v2)
    mode = "mixed" if m1 != m2 else m1
    return ok, mode, v1, v2
...
def main():
    ...
    with engine.begin() as conn:
        ...
        """, conn)
        if canon.empty:
            print(json.dumps({"clean_rows": 0, "note": "no canonical clusters"}, ensure_ascii=False)); return

        canon["member_internal_ids"] = canon["member_internal_ids"].apply(
            lambda x: x if isinstance(x, list) else []
        )
        canon = canon.explode("member_internal_ids").dropna()
        canon["internal_id"] = pd.to_numeric(canon["member_internal_ids"], errors="coerce").astype("Int64")
        ...
        df = pd.read_sql(f"""
            ...
            ORDER BY s.internal_id, s.ingested_at DESC NULLS LAST,
                     s.archived_date DESC NULLS LAST, s.added_date DESC NULLS LAST
        """, conn)

        if df.empty:
            print(json.dumps({"clean_rows": 0, "note": "empty summarized_geo"}, ensure_ascii=False)); return
        ...
        facts_index: set[int] = set(map(int, facts.index.tolist()))
        missing_ids_total = 0
        pairs_checked_total = 0

        rows = []
        for canon_id, g in canon.groupby("canonical_internal_id"):
            raw_ids = pd.to_numeric(g["internal_id"], errors="coerce")
            raw_ids = raw_ids[pd.notna(raw_ids)].astype("int64").tolist()

            if len(raw_ids) < 2:
                continue

            ids = [i for i in raw_ids if int(i) in facts_index]
            missing_here = len(raw_ids) - len(ids)
            if missing_here:
                missing_ids_total += missing_here
            ...
            for a, b in itertools.combinations(ids, 2):
                ...
                # GEO уровень сопоставления
                lvl = _geo_level(r1, r2)
                if lvl is None:
                    continue

                same_cat  = (pd.notna(r1.category_value) and pd.notna(r2.category_value)
                             and int(r1.category_value) == int(r2.category_value))
                same_deal = (str(r1.deal_type) == str(r2.deal_type))
                if not (same_cat and same_deal):
                    continue

                price_ok, phi, plo = _price_check(r1.price, r2.price, PRICE_RATIO)
        ...
        if not clean.empty:
            ...
            clean.drop(columns=["__lvl_r", "__pr_r", "__area_r"], inplace=True, errors="ignore")
            dedup_dropped = int(before - len(clean))
        else:
            dedup_dropped = 0
        ...
        if not clean.empty:
            clean.to_sql(
                T_TMP, con=conn, if_exists="append", index=False, method="multi", chunksize=50_000,
                dtype={
                    "canonical_internal_id": BIGINT(),
        ...
        print(json.dumps({
            ...
            "rules": {
                ...
                "land_cats": sorted(list(CAT_LAND_VALUES))
            },
