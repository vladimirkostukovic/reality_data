...
TABLE_IMAGE_DUP = "image_duplicate"            # site_id1, site_id2, percentage, date
TABLE_SUMMARY   = "summarized_geo"             # site_id, internal_id, added_date (или date)

TABLE_MAP       = "internal_id_canon_map"      # internal_id -> canonical_internal_id
TABLE_CANON     = "image_duplicate_canonical"  # агрегат на кластер (канон)
# ============================================================
    ...
    with engine.begin() as conn:
        ...
        if "added_date" not in geo.columns or geo["added_date"].isna().all():
            ...
            if dc:
                ...
                raise RuntimeError("summarized_geo без added_date/date — не выбрать канон.")
        ...
        # map site_id -> internal_id (LEFT + dropna)
        dup = dup.merge(
        ...
        cluster_edges = (dup.assign(canonical_internal_id=np.where(dup["cid1"] <= dup["cid2"], dup["cid1"], dup["cid2"]))
                           .groupby("canonical_internal_id")
                           .agg(first_seen=("date","min"),
                                last_seen=("date","max"),
                                max_percentage=("percentage","max"),
                                edges_count=("percentage","size"),
                                site_ids=("site_id1", lambda s: list(s.astype(str)))  # временно
                                )
                           .reset_index())
        ...
        cluster_edges["site_ids"] = cluster_edges.apply(
            lambda r: sorted(set((r["site_ids"] or []) + (r["site_ids2"] or []))), axis=1
        )
        cluster_edges = cluster_edges.drop(columns=["site_ids2"])

        members = (map_df.groupby("canonical_internal_id")
                         .agg(members_count=("internal_id","size"),
                              member_internal_ids=("internal_id", lambda s: sorted(list(map(int, set(s))))))
                         .reset_index())

        canon_agg = (members.merge(cluster_edges, on="canonical_internal_id", how="left")
        ...
        print(json.dumps({
            ...
            "pairs_kept_both_in_geo": int(pairs_kept),
            "map_rows": int(len(map_df)),
            "canon_rows": int(len(canon_agg)),
            "target_map": f"{SCHEMA_SILVER}.{TABLE_MAP}",
            "target_canon": f"{SCHEMA_SILVER}.{TABLE_CANON}"
        }, ensure_ascii=False))
