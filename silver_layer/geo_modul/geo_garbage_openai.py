#This script processes invalid or incomplete geo records in silver.geo_garbage by sending
# them to the OpenAI Batch API for normalization.
#It updates the cleaned fields back into the table, marks corrected rows as valid,
# and produces a strict ETL sync log.

from __future__ import annotations
import os, sys, json, time, tempfile
from pathlib import Path
from typing import Any, Dict, List, Optional, Tuple, Callable

import pandas as pd
from sqlalchemy import create_engine, text
from sqlalchemy.engine import Engine

from openai import OpenAI
from openai._exceptions import APIStatusError, APIConnectionError, RateLimitError


# ---------- CONFIG ----------
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parents[1]

with open(PROJECT_ROOT / "config.json", "r", encoding="utf-8") as f:
    cfg = json.load(f)


def _cfg_get(*path, default=None):
    cur = cfg
    for k in path:
        if isinstance(cur, dict) and k in cur:
            cur = cur[k]
        else:
            return default
    return cur


DB_URL = (
    f"postgresql+psycopg2://"
    f"{cfg['USER']}:{cfg['PWD']}@{cfg['HOST']}:{cfg['PORT']}/{cfg['DB']}"
)
engine: Engine = create_engine(DB_URL, pool_pre_ping=True, future=True)


API_KEY = (
    os.getenv("OPENAI_API_KEY")
    or _cfg_get("OPENAI_API_KEY")
    or _cfg_get("OPENAI", "API_KEY")
)
if not API_KEY:
    print(json.dumps({"stage": "geo_garbage_batchsync", "status": "fail", "error": "no_openai_api_key"}))
    sys.exit(1)

client = OpenAI(api_key=API_KEY)


PROMPT_TEMPLATE = (
    _cfg_get("PROMPTS", "GEO_GARBAGE_TEMPLATE_V2")
    or _cfg_get("PROMPTS", "CZ_ADDRESS_TEMPLATE_V2")
)
if not PROMPT_TEMPLATE:
    print(json.dumps({"stage": "geo_garbage_batchsync", "status": "fail", "error": "no_prompt_in_config"}))
    sys.exit(1)


BATCH = _cfg_get("BATCH_GEO", default={}) or {}
CHUNK_SIZE = int(BATCH.get("CHUNK_SIZE", 500))
POLL_SEC = int(BATCH.get("POLL_SEC", 60))
MODEL = BATCH.get("MODEL", "gpt-4o-mini")
FILTER_NULL_IN_CZ = bool(BATCH.get("FILTER_NULL_IN_CZ", True))
MAX_ROWS = BATCH.get("MAX_ROWS")
MAX_ACTIVE = int(BATCH.get("MAX_ACTIVE", 7))
FAIL_STOP_N = int(BATCH.get("FAIL_STOP_N", 8))

if MAX_ROWS is not None:
    try:
        MAX_ROWS = int(MAX_ROWS)
    except:
        MAX_ROWS = None


# ---------- RETRY ----------
def _retry(n: int, pause: float, fn: Callable, *args, **kwargs):
    last = None
    for _ in range(n):
        try:
            return fn(*args, **kwargs)
        except Exception as e:
            last = e
            time.sleep(pause)
    raise last


# ---------- LOAD ----------
def _load_candidates(limit: Optional[int] = None) -> pd.DataFrame:
    where_extra = "AND g.in_cz IS NULL" if FILTER_NULL_IN_CZ else ""
    sql = f"""
        SELECT internal_id, district, city, city_part, street
        FROM silver.geo_garbage g
        WHERE g.geo_status IS FALSE
        {where_extra}
        ORDER BY g.internal_id
    """
    if limit:
        sql += f" LIMIT {int(limit)}"

    with engine.begin() as conn:
        conn.execute(text("SET LOCAL statement_timeout = '120s'"))
        return pd.read_sql(sql, con=conn)


# ---------- HELPERS ----------
def _s(val) -> str:
    if pd.isna(val):
        return ""
    s = str(val).strip()
    return "" if s.lower() in {"none", "nan", "null", "neuvedeno", "neznámá"} else s


_VALID_KEYS = ("district", "city", "city_part", "street")
_LEGACY_MAP = {
    "norm_district": "district",
    "norm_okres": "district",
    "norm_city": "city",
    "norm_city_part": "city_part",
    "norm_street": "street",
}


def _render_prompt(template: str, values: Dict[str, str]) -> str:
    safe_tpl = template.replace("{", "{{").replace("}", "}}")

    for k in _VALID_KEYS:
        safe_tpl = safe_tpl.replace("{{" + k + "}}", "{" + k + "}")

    for old, new in _LEGACY_MAP.items():
        safe_tpl = safe_tpl.replace("{{" + old + "}}", "{" + new + "}")

    return safe_tpl.format(
        district=values.get("district", ""),
        city=values.get("city", ""),
        city_part=values.get("city_part", ""),
        street=values.get("street", ""),
    )


# ---------- TEMP JSONL ----------
def _tmp_file(suffix=".jsonl") -> Path:
    return Path(tempfile.NamedTemporaryFile(delete=False, suffix=suffix).name)


def _write_jsonl_for_chunk(df: pd.DataFrame) -> Tuple[Path, int]:
    path = _tmp_file(".jsonl")
    written = 0

    with open(path, "w", encoding="utf-8") as f:
        for _, r in df.iterrows():
            prompt = _render_prompt(PROMPT_TEMPLATE, {
                "district": _s(r.get("district")),
                "city": _s(r.get("city")),
                "city_part": _s(r.get("city_part")),
                "street": _s(r.get("street")),
            })

            job = {
                "custom_id": str(int(r["internal_id"])),
                "method": "POST",
                "url": "/v1/chat/completions",
                "body": {
                    "model": MODEL,
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": 200,
                    "temperature": 0
                }
            }

            f.write(json.dumps(job, ensure_ascii=False) + "\n")
            written += 1

    return path, written


# ---------- BATCH OPS ----------
def _upload_and_start(infile: Path) -> Tuple[str, str]:
    with open(infile, "rb") as fh:
        file_obj = _retry(3, 2.0, client.files.create, file=fh, purpose="batch")

    batch = _retry(
        3, 2.0, client.batches.create,
        input_file_id=file_obj.id,
        endpoint="/v1/chat/completions",
        completion_window="24h"
    )
    return batch.id, file_obj.id


def _get_batch(bid: str):
    return _retry(3, 2.0, client.batches.retrieve, bid)


def _download(file_id: str) -> bytes:
    return _retry(3, 2.0, lambda: client.files.content(file_id).read())


# ---------- PARSE ----------
def _parse_choice(obj: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        body = (obj.get("response") or {}).get("body") or {}
        choices = body.get("choices") or []
        if not choices:
            return None

        content = choices[0]["message"]["content"]
        if isinstance(content, dict):
            return content
        if isinstance(content, str):
            try:
                return json.loads(content.strip())
            except:
                return None
    except:
        return None


# ---------- DB ----------
def _coerce_bool(v):
    if v is None:
        return None
    s = str(v).strip().lower()
    if s in {"true", "1", "yes"}:
        return True
    if s in {"false", "0", "no"}:
        return False
    return None


def _ensure_index():
    with engine.begin() as conn:
        conn.execute(text("""
            CREATE INDEX IF NOT EXISTS geo_garbage_internal_id_idx
            ON silver.geo_garbage (internal_id);
        """))


def _apply_rows_to_db(rows: List[Dict[str, Any]]) -> Tuple[int, int, int]:
    if not rows:
        return (0, 0, 0)

    data = []
    icz_true = icz_false = 0

    for r in rows:
        cid = int(r["custom_id"])
        p = r.get("parsed") or {}
        in_cz = _coerce_bool(p.get("in_cz"))

        if in_cz is True:
            icz_true += 1
        elif in_cz is False:
            icz_false += 1

        data.append({
            "internal_id": cid,
            "norm_district": p.get("norm_district"),
            "norm_okres": p.get("norm_okres"),
            "norm_city": p.get("norm_city"),
            "norm_city_part": p.get("norm_city_part"),
            "norm_street": p.get("norm_street"),
            "in_cz": in_cz
        })

    payload_json = json.dumps(data, ensure_ascii=False).replace("'", "''")

    sql = f"""
        WITH src AS (
            SELECT
                (x->>'internal_id')::bigint AS internal_id,
                NULLIF(x->>'norm_district','') AS norm_district,
                NULLIF(x->>'norm_okres','') AS norm_okres,
                NULLIF(x->>'norm_city','') AS norm_city,
                NULLIF(x->>'norm_city_part','') AS norm_city_part,
                NULLIF(x->>'norm_street','') AS norm_street,
                CASE WHEN x ? 'in_cz' THEN
                    CASE LOWER(x->>'in_cz')
                        WHEN 'true' THEN TRUE
                        WHEN '1' THEN TRUE
                        WHEN 'yes' THEN TRUE
                        WHEN 'false' THEN FALSE
                        WHEN '0' THEN FALSE
                        WHEN 'no' THEN FALSE
                        ELSE NULL
                    END
                END AS in_cz
            FROM jsonb_array_elements('{payload_json}'::jsonb) AS x
        )
        UPDATE silver.geo_garbage AS tgt
        SET
            norm_district  = COALESCE(src.norm_district, tgt.norm_district),
            norm_okres     = COALESCE(src.norm_okres, tgt.norm_okres),
            norm_city      = COALESCE(src.norm_city, tgt.norm_city),
            norm_city_part = COALESCE(src.norm_city_part, tgt.norm_city_part),
            norm_street    = COALESCE(src.norm_street, tgt.norm_street),
            in_cz          = COALESCE(src.in_cz, tgt.in_cz),
            geo_status     = TRUE
        FROM src
        WHERE tgt.internal_id = src.internal_id;
    """

    with engine.begin() as conn:
        conn.execute(text("SET LOCAL statement_timeout = '120s'"))
        res = conn.execute(text(sql))

    return int(res.rowcount or 0), icz_true, icz_false


# ---------- MAIN ----------
def main():
    _ensure_index()
    t0 = time.perf_counter()

    summary = {
        "stage": "geo_garbage_batchsync",
        "status": "ok",
        "chunks": 0,
        "rows_total": 0,
        "rows_sent": 0,
        "rows_rcv": 0,
        "rows_updated": 0,
        "in_cz_true": 0,
        "in_cz_false": 0,
        "in_cz_null": 0,
        "consecutive_failures": 0,
        "aborted_due_to_failures": False,
        "elapsed_s": 0.0,
    }

    try:
        df = _load_candidates(MAX_ROWS)
        total = len(df)
        summary["rows_total"] = total

        if total == 0:
            summary["elapsed_s"] = round(time.perf_counter() - t0, 3)
            print(json.dumps(summary, ensure_ascii=False))
            return

        chunks = [df.iloc[i:i + CHUNK_SIZE] for i in range(0, total, CHUNK_SIZE)]
        summary["chunks"] = len(chunks)

        queue = list(enumerate(chunks, start=1))
        active = {}
        consec = 0

        while queue or active:

            while queue and len(active) < MAX_ACTIVE:
                idx, cdf = queue.pop(0)

                infile, rows = _write_jsonl_for_chunk(cdf)
                summary["rows_sent"] += rows

                bid, fid = _upload_and_start(infile)
                infile.unlink(missing_ok=True)

                active[bid] = {"idx": idx, "rows": rows, "file_id": fid}

            if not active:
                break

            time.sleep(POLL_SEC)

            finished = []
            for bid, meta in active.items():
                b = _get_batch(bid)
                st = getattr(b, "status", "unknown")
                if st in ("completed", "failed", "expired", "cancelled"):
                    meta["status"] = st
                    meta["output_file_id"] = getattr(b, "output_file_id", None)
                    finished.append(bid)

            for bid in finished:
                meta = active.pop(bid)
                status = meta["status"]

                if status != "completed":
                    consec += 1
                    summary["consecutive_failures"] = consec

                    if consec >= FAIL_STOP_N:
                        summary["status"] = "fail"
                        summary["aborted_due_to_failures"] = True
                        queue.clear()
                        active.clear()
                        break

                    continue

                consec = 0
                summary["consecutive_failures"] = 0

                out_id = meta.get("output_file_id")
                if not out_id:
                    continue

                content = _download(out_id)
                parsed = []

                for line in content.splitlines():
                    try:
                        obj = json.loads(line.decode() if isinstance(line, (bytes, bytearray)) else line)
                        cid = obj.get("custom_id")
                        parsed_obj = _parse_choice(obj)
                        if cid and isinstance(parsed_obj, dict):
                            parsed.append({"custom_id": cid, "parsed": parsed_obj})
                    except:
                        pass

                summary["rows_rcv"] += len(parsed)

                updated, ctrue, cfalse = _apply_rows_to_db(parsed)
                summary["rows_updated"] += updated
                summary["in_cz_true"] += ctrue
                summary["in_cz_false"] += cfalse

            if summary["aborted_due_to_failures"]:
                break

        summary["in_cz_null"] = max(
            0,
            summary["rows_rcv"] - summary["in_cz_true"] - summary["in_cz_false"]
        )
        summary["elapsed_s"] = round(time.perf_counter() - t0, 3)

        if summary["rows_sent"] != summary["rows_rcv"]:
            summary["status"] = "fail"

        out = {
            "stage": "geo_garbage_batchsync",
            "status": summary["status"],
            "summary": {
                "chunks": summary["chunks"],
                "rows_total": summary["rows_total"],
                "rows_sent": summary["rows_sent"],
                "rows_received": summary["rows_rcv"],
                "rows_updated": summary["rows_updated"],
                "in_cz": {
                    "true": summary["in_cz_true"],
                    "false": summary["in_cz_false"],
                    "null": summary["in_cz_null"]
                },
                "failures": {
                    "consecutive": summary["consecutive_failures"],
                    "aborted": summary["aborted_due_to_failures"]
                },
                "elapsed_s": summary["elapsed_s"]
            }
        }

        print(json.dumps(out, ensure_ascii=False))

    except Exception as e:
        print(json.dumps({
            "stage": "geo_garbage_batchsync",
            "status": "fail",
            "error": f"{e.__class__.__name__}: {e}"
        }))
        sys.exit(1)


if __name__ == "__main__":
    main()